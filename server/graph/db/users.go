...
type userR struct {
	...
	Chills            ChillSlice           `boil:"Chills" json:"Chills" toml:"Chills" yaml:"Chills"`
...
func (r *userR) GetChills() ChillSlice {
	if r == nil {
		return nil
	}
	return r.Chills
...
// Chills retrieves all the chill's Chills with an executor.
func (o *User) Chills(mods ...qm.QueryMod) chillQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"chilly_daze\".\"chills\".\"user_id\"=?", o.ID),
	)
...
// LoadChills allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadChills(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`chilly_daze.chills.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load chills")
	}

	var resultSlice []*Chill
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice chills")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on chills")
	}
	if err = results.Err(); err != nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Chills = append(local.R.Chills, foreign)
				if foreign.R == nil {
					foreign.R = &chillR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserAchievements(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
...
// AddChills adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Chills.
// Sets related.R.User appropriately.
func (o *User) AddChills(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Chill) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.Chills = append(o.R.Chills, related...)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddUserAchievements(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserAchievement) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
